...
# no XZ codec are on the classpath, and the FindBugs annotations are compile-time
# only. A -keep does not silence a dangling reference, only -dontwarn does, so
# these are the rules that let R8 finish.
-dontwarn org.slf4j.**
-dontwarn org.osgi.**
